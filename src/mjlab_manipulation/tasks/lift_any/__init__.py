...
from mjlab_manipulation.tasks.lift.rl_cfg import lift_vision_ppo_runner_cfg

from .env_cfgs import lift_any_vision_env_cfg
...
for name, robot_fn in _ROBOTS.items():
  exp = f"lift_any_{name.lower()}_rgb"
  register_mjlab_task(
    task_id=f"Lift-Any-{name}-RGB",
    env_cfg=lift_any_vision_env_cfg(cam_type="rgb", robot_cfg_fn=robot_fn),
    play_env_cfg=lift_any_vision_env_cfg(
      cam_type="rgb", play=True, robot_cfg_fn=robot_fn
    ),
    rl_cfg=lift_vision_ppo_runner_cfg(experiment_name=exp),
